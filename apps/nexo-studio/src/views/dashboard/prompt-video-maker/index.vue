...
async function handleSavePrompt() {
  ...
  // Đảm bảo finalPrompt.value là một chuỗi trước khi gán
  const finalPromptString = finalPrompt.value || '';

  const newPrompt: UserPrompt = {
    id: nanoid(),
    templateId: promptTemplate.value.id,
    filledValues: selections.value,
    finalPrompt: finalPromptString,
    createdAt: new Date(),
  };

  try {
    await db.userPrompts.add(newPrompt);
    message.success('Đã lưu prompt thành công!');
  } catch (error) {
    console.error('Lỗi khi lưu prompt:', error);
    message.error('Có lỗi xảy ra khi lưu prompt.');
  }
}
</script>

<template>
  <Page title="Prompt Video Maker">
    <Row :gutter="16">
      <Col :span="12">
      ...
      <Col :span="12">
        <Card title="Prompt Hoàn Chỉnh">
          ...
          <template #actions>
            <Button type="primary" @click="handleSavePrompt">
              Lưu Prompt
            </Button>
          </template>
        </Card>
