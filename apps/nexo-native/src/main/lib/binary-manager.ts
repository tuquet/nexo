...
async function checkBinariesExist(): Promise<boolean> {
  try {
    ...
    log.debug(
      `[BinaryManager] Tất cả công cụ đã được tìm thấy và có thể thực thi.${binariesPath}`,
    );
    return true;
  } catch {
    log.warn(
      '[BinaryManager] Một hoặc nhiều công cụ bị thiếu hoặc không thể thực thi.',
    );
    return false;
  }
}
...
function getLatestFfbinariesVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    ffbinaries.listVersions((err: Error | null, versions: string[]) => {
      if (err) {
        log.error(
          '[BinaryManager] Failed to get versions from ffbinaries.',
          err,
        );
        return reject(err);
      }
    ...
    ffbinaries.downloadFiles(
      BINARIES,
      {
        ...
        tickerFn: (progress) => {
          ...
          log.info(
            `[BinaryManager] Đang tải: ${percent}% của ${progress.filename}`,
          );
          mainWindow?.webContents.send('binary-manager:status', {
            status: 'downloading',
      ...
      async (err) => {
        ...
        try {
          ...
          log.info(
            `[BinaryManager] Đã ghi manifest thành công cho phiên bản ${versionToDownload}.`,
          );
          resolve();
        } catch (manifestError) {
          log.error(
            '[BinaryManager] Không thể ghi manifest phiên bản sau khi tải.',
            manifestError,
          );
...
export async function ensureBinaries(): Promise<Record<BinaryName, string>> {
  ...
  if (filesExist) {
    ...
    try {
      ...
      log.warn(
        '[BinaryManager] Không thể đọc manifest phiên bản. Bắt đầu tải lại.',
        error,
      );
      shouldDownload = true;
      await fs.rm(binariesPath, { recursive: true, force: true });
  ...
  if (shouldDownload) {
    ...
    if (!latestVersion) {
      try {
        ...
        throw new Error(
          `Could not determine which version to download: ${error.message}`,
        );
      }
    }
