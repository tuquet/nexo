...
export function Video(ipc: IpcMain): void {
  ...
  ipc.handle(
    ...
    async (
      ...
      try {
        ...
        const isPlaylist =
          videoUrl.includes('list=') && options?.downloadPlaylist;
        ...
        return new Promise((resolve, reject) => {
          process.on('close', (code) => {
            ...
            const isCookieError =
              stderrData.includes('could not find') &&
              stderrData.includes('cookies database');
            ...
            if (stdoutData.trim() && (code === 0 || isCookieError)) {
              try {
                ...
                log.info(
                  `[Video] Fetched info for ${results.length} item(s) from URL.`,
                );
                resolve(results);
              } catch (error) {
                log.error(
              ...
              log.error(
                `[Video] yt-dlp exited with code ${code}. Stderr: ${stderr}`,
              );
              ...
              if (stderr.includes('Unsupported URL')) {
                ...
                stderr.includes('are needed')
              ) {
                reject(
                  new Error(
                    JSON.stringify({ code: 'COOKIES_NEEDED', message: stderr }),
                  ),
                );
              } else if (
                ...
                reject(
                  new Error(
                    JSON.stringify({ code: 'FETCH_FAILED', message: stderr }),
                  ),
                );
              }
            }
        ...
        throw new Error(
          JSON.stringify({ code: 'PROCESS_ERROR', message: errorMessage }),
        );
      }
    },
  );
  ...
  ipc.handle(
    ...
    async (
      ...
      try {
        ...
        const [binaries, ytdlp] = await Promise.all([
          ensureBinaries(),
          ensureYtDlp(),
        ]);
      ...
      const outputTemplate: string = isAudioOnly
        ? path.join(outputPath, '%(title)s.%(ext)s') // Đối với audio, template đơn giản là tốt nhất. Phần mở rộng sẽ là mp3.
        : path.join(outputPath, '%(title)s-%(height)sp.%(ext)s'); // Thay thế " [%(height)sp]" bằng "-%(height)sp" để có tên file thân thiện hơn.
      ...
      return new Promise((resolve, reject) => {
        ...
        if (isAudioOnly) {
          log.info(
            '[Video] Audio only download requested. Setting format to mp3.',
          );
          args.push(
            '--extract-audio',
            '--audio-format',
          ...
          log.info(
            '[Video] No format code selected, using default best quality.',
          );
          args.push(
            '--format',
        ...
        let videoTitle: null | string = null;
        ...
        const handleDataChunk = (text: string): void => {
          ...
          if (allProgressMatches) {
            const lastMatchText =
              allProgressMatches[allProgressMatches.length - 1];
        ...
        subprocess.on('close', (code, signal) => {
          ...
          if (canceledJobs.has(jobId)) {
            ...
            log.info(
              `[Video] Download for ${videoUrl} was canceled by the user. Cleaning up...`,
            );
            ...
            const destinationMatches = [
              ...fullOutput.matchAll(destinationRegexGlobal),
            ];
          ...
          if (signal) {
            log.info(
              `[Video] Download for ${videoUrl} was terminated by signal: ${signal}.`,
            );
            return reject(
              new Error(
                JSON.stringify({
          ...
          const alreadyDownloadedMatch = fullOutput.match(
            alreadyDownloadedRegex,
          );
          if (alreadyDownloadedMatch && alreadyDownloadedMatch[1]) {
            const existingPath = alreadyDownloadedMatch[1].trim();
            log.info(
              `[Video] Bỏ qua tải xuống, file đã tồn tại: ${existingPath}`,
            );
            if (!videoTitle) {
          ...
          const destinationMatches = [
            ...fullOutput.matchAll(new RegExp(destinationRegex, 'g')),
          ];
          ...
          log.error(
            `[Video] Process exited with code: ${code}. Stderr: ${fullStderr}`,
          );

          if (
            fullStderr.includes('Fresh cookies') &&
            fullStderr.includes('are needed')
          ) {
            return reject(
              new Error(
                JSON.stringify({
          ...
          if (
            useCookieFile &&
            fullStderr.toLowerCase().includes('http error 403')
          ) {
            return reject(
              new Error(
                JSON.stringify({
          ...
          const hasCookieError =
            fullStderr.includes('could not find') &&
            fullStderr.includes('cookies database');
          const hasOtherFatalErrors = fullStderr.split('\n').some((line) => {
            ...
            return (
              ...
              !(
                lowerLine.includes('could not find') &&
                lowerLine.includes('cookies database')
              )
            );
          });
          ...
          const errorMessage =
            fullStderr.trim() || `Download failed with exit code: ${code}.`;
          reject(
            new Error(
              JSON.stringify({
        ...
        subprocess.on('error', (error) => {
          ...
          reject(
            new Error(
              JSON.stringify({ code: 'PROCESS_ERROR', message: error.message }),
            ),
          );
        });
      });
  ...
  ipc.handle('video:stop-download', async (_, jobId: string) => {
    ...
    if (subprocess && subprocess.pid) {
      ...
      log.info(
        `[Video] Received request to stop download for job: ${jobId}, PID: ${pid}`,
      );
      ...
      if (process.platform === 'win32') {
        ...
        exec(`taskkill /PID ${pid} /T /F`, { windowsHide: true }, (err) => {
          if (err) {
            ...
            log.info(
              `[Video] Successfully issued taskkill for process tree of PID ${pid}.`,
            );
          }
        });
      } else {
        ...
        try {
          ...
          log.info(
            `[Video] Successfully sent SIGKILL to process group -${pid}.`,
          );
        } catch (error: any) {
