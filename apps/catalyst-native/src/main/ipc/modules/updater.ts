...
export function updater(ipc: IpcMain): void {
  ...
  autoUpdater.on('download-progress', (progressInfo) => {
    log.debug(`[Updater] Download progress: ${progressInfo.percent.toFixed(2)}%`)
  ...
  ipc.on('updater:clear-cache', async () => {
    ...
    try {
      ...
      const downloadedUpdateHelper = await (autoUpdater as any).getOrCreateDownloadHelper()
    ...
    if (!cacheDir) {
      log.error('[Updater] Cache directory path is not available. Cannot clear cache.')
      return
    }
    ...
    try {
      ...
      log.info('[Updater] Cache directory cleared successfully (or it did not exist).')
    } catch (error) {
      log.error('[Updater] An unexpected error occurred while clearing the updater cache:', error)
    }
  })
}
