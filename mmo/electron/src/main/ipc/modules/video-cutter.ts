...
export function videoCutter(ipc: IpcMain): void {
  ipc.handle(
    ...
    async (
      ...
      log.info(`[VideoCutter] Starting video cutting process for job: ${jobId}...`)
      ...
      try {
        ...
        log.info('[VideoCutter] Verifying required binaries (FFmpeg/FFprobe)...')
        const binaries = await ensureBinaries(['ffmpeg', 'ffprobe'])
      ...
      try {
        ...
        const getDuration = new Promise<string>((resolve, reject) => {
          ...
          ffprobeProcess.on('close', (code) =>
            code === 0 ? resolve(stdout.trim()) : reject(new Error('ffprobe failed'))
          )
        ...
        log.info('[VideoCutter] Using precise cut mode (re-encoding with forced keyframes).')
          ...
          child.on('close', (code) => {
            ...
            if (code === 0) {
              ...
              const cleanError =
                fullError
                  .split('\n')
                  .filter((line) => line.trim() && line.toLowerCase().includes('error'))
                  .pop() || `FFmpeg process exited with code ${code}`

              log.error(`[VideoCutter] Job ${jobId} failed. Full error: ${fullError}`)
  ...
  ipc.handle('video:stop-cut', async (_, jobId: string) => {
    ...
    if (subprocess && subprocess.pid) {
      ...
      log.info(`[VideoCutter] Received request to stop cut for job: ${jobId}, PID: ${pid}`)
      canceledCuts.add(jobId)

      if (process.platform === 'win32') {
        exec(`taskkill /PID ${pid} /T /F`, { windowsHide: true }, (err) => {
          if (err) {
            log.warn(`[VideoCutter] taskkill for PID ${pid} failed:`, err.message)
          } else {
            log.info(`[VideoCutter] Successfully issued taskkill for process tree of PID ${pid}.`)
          }
        })
      } else {
        try {
          ...
          log.info(`[VideoCutter] Successfully sent SIGKILL to process group -${pid}.`)
        } catch (err: any) {
          log.warn(`[VideoCutter] process.kill for group -${pid} failed:`, err.message)
        }
      }
      return { success: true }
    }
